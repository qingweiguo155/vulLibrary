  ...
  export default {
    ...
    mounted() {
      ...
      this.$cy = cytoscape({
        ...
        zoom: 2, // 图表的初始缩放级别.可以设置options.minZoom和options.maxZoom设置缩放级别的限制.
    ...
    // minNodeSpacing: 100 // min spacing between outside of nodes (used for radius adjustment)
      ...
      this.$cy.style()
        /*未选择节点样式*/
        .selector('node')
        .style({'label': 'data(name)', 'font-size': '10pt', 'width': '30pt', 'height': '30pt'})
        /*已选择节点样式*/
    ...
    methods: {
      ...
      //       let elements = ((Array.isArray ? Array.isArray(eles) : null != eles && eles instanceof Array) ? eles : [eles]);
            // let filterElements = eles.filter(__ => !this.$cy.getElementById(__.data.id).length)
            this.$cy.add(eles);
      //     });
      //     this.$cy.endBatch();
      //   }
